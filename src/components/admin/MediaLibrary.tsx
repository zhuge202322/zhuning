...
export default function MediaLibrary() {
  ...
  const loadAssets = useCallback(async ({ cursor = null, append = false, signal }: { cursor?: string | null; append?: boolean; signal?: AbortSignal } = {}) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(data.error || "Failed to load media assets");
      ...
      if (!(caught instanceof DOMException && caught.name === "AbortError")) setError(caught instanceof Error ? caught.message : "Failed to load media assets");
  ...
  async function saveAlt() {
    ...
    try {
      ...
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || "Failed to update alt text");
      setAssets((current) => current.map((asset) => asset.id === data.asset.id ? data.asset : asset));
      setEditing(null);
    } catch (caught) { setError(caught instanceof Error ? caught.message : "Failed to update alt text"); }
    finally { setBusyId(null); }
  }
  ...
  async function replace(file: File) {
    ...
    try {
      ...
      setAssets((current) => current.map((asset) => asset.id === data.asset.id ? data.asset : asset));
      setReplacing(null);
    } catch (caught) { setError(caught instanceof Error ? caught.message : "Failed to replace media"); }
    finally { setBusyId(null); }
  }
  ...
  async function remove(asset: MediaAsset) {
    ...
    } catch (caught) { setError(caught instanceof Error ? caught.message : "Failed to delete media"); }
    finally { setBusyId(null); }
  }
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div><h2 className="text-xl font-bold text-slate-900">Media library</h2><p className="mt-1 text-sm text-slate-500">Upload and reuse secure local images and videos.</p></div>
        <button type="button" onClick={() => uploadRef.current?.click()} disabled={busyId === "upload"} className="inline-flex items-center gap-2 rounded-md bg-brand-primary px-4 py-2 text-sm font-bold text-white disabled:opacity-50">{busyId === "upload" ? <Loader2 className="h-4 w-4 animate-spin" /> : <Upload className="h-4 w-4" />}{busyId === "upload" ? "Uploading..." : "Upload media"}</button>
      </div>
      ...
      <div className="flex flex-wrap gap-3 border-y border-slate-200 py-4">
        <label className="relative min-w-64 flex-1"><Search className="absolute left-3 top-2.5 h-4 w-4 text-slate-400" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search filename or alt text" className="w-full rounded-md border border-slate-300 py-2 pl-9 pr-3 text-sm outline-none focus:border-brand-primary" /></label>
        <div className="inline-flex rounded-md border border-slate-300 bg-white p-1">{(["all", "image", "video"] as const).map((option) => <button key={option} type="button" onClick={() => setType(option)} className={`rounded px-3 py-1.5 text-sm font-semibold capitalize ${type === option ? "bg-slate-900 text-white" : "text-slate-600 hover:bg-slate-100"}`}>{option}</button>)}</div>
        <button type="button" onClick={() => void loadAssets()} title="Refresh media" className="rounded-md border border-slate-300 bg-white p-2 text-slate-600 hover:bg-slate-50"><RefreshCw className="h-4 w-4" /></button>
      </div>
      ...
      {loading ? <div className="flex items-center justify-center gap-2 py-20 text-sm text-slate-500"><Loader2 className="h-4 w-4 animate-spin" />Loading media assets...</div> : null}
      {!loading && !error && assets.length === 0 ? <div className="border-y border-slate-200 py-20 text-center"><ImageIcon className="mx-auto h-8 w-8 text-slate-300" /><p className="mt-3 text-sm font-semibold text-slate-600">No media assets found</p><p className="mt-1 text-xs text-slate-400">Upload a file or change your search filters.</p></div> : null}
      {!loading && assets.length ? <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{assets.map((asset) => (
        <article key={asset.id} className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
          ...
          <div className="space-y-3 p-3"><div><p className="truncate text-sm font-bold text-slate-900" title={asset.originalName}>{asset.originalName}</p><p className="mt-0.5 text-xs text-slate-500">{asset.width && asset.height ? `${asset.width} x ${asset.height} · ` : ""}{(asset.byteSize / 1024).toFixed(1)} KB</p><p className="mt-1 truncate text-xs text-slate-500">{asset.alt || "No alt text"}</p></div>
            <div className="grid grid-cols-4 gap-1 border-t border-slate-100 pt-2">
              <button type="button" onClick={() => void copyUrl(asset)} title="Copy URL" className="flex justify-center rounded-md p-2 text-slate-600 hover:bg-slate-100">{copiedId === asset.id ? <Check className="h-4 w-4 text-emerald-600" /> : <Clipboard className="h-4 w-4" />}</button>
              <button type="button" onClick={() => setEditing({ ...asset })} title="Edit alt text" className="flex justify-center rounded-md p-2 text-slate-600 hover:bg-slate-100"><Pencil className="h-4 w-4" /></button>
              <button type="button" onClick={() => { setReplacing(asset); replaceRef.current?.click(); }} title="Replace file" className="flex justify-center rounded-md p-2 text-slate-600 hover:bg-slate-100"><RefreshCw className="h-4 w-4" /></button>
              <button type="button" disabled={busyId === asset.id} onClick={() => void remove(asset)} title="Delete media" className="flex justify-center rounded-md p-2 text-red-600 hover:bg-red-50 disabled:opacity-50">{busyId === asset.id ? <Loader2 className="h-4 w-4 animate-spin" /> : <Trash2 className="h-4 w-4" />}</button>
            </div>
          </div>
        </article>
      ))}</div> : null}
      ...
      {editing ? <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/55 p-4"><div className="w-full max-w-md rounded-lg bg-white p-5 shadow-2xl"><h3 className="font-bold text-slate-900">Edit alt text</h3><p className="mt-1 truncate text-xs text-slate-500">{editing.originalName}</p><textarea autoFocus rows={4} value={editing.alt} onChange={(event) => setEditing({ ...editing, alt: event.target.value })} className="mt-4 w-full rounded-md border border-slate-300 p-3 text-sm outline-none focus:border-brand-primary" /><div className="mt-4 flex justify-end gap-2"><button type="button" onClick={() => setEditing(null)} className="rounded-md border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700">Cancel</button><button type="button" onClick={() => void saveAlt()} disabled={busyId === editing.id} className="rounded-md bg-brand-primary px-4 py-2 text-sm font-bold text-white disabled:opacity-50">Save</button></div></div></div> : null}
    </div>
  );
}
